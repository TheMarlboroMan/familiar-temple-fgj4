...
void Nivel::convertir_celda_a_deco(unsigned int indice, unsigned int tile)
{
	auto insertar=[this](unsigned int indice, unsigned int tile)
	{
		Coordenadas coor=indice_a_coordenadas(indice);
		Celda_deco * celda=new Celda_deco(coor.x, coor.y, tile);
		rejilla.insert(std::make_pair(indice, std::unique_ptr<Celda>(celda) ) );
	};
...
Nivel::Coordenadas Nivel::obtener_coordenadas_inicio_en_rejilla() const
{
	auto resultado=std::find_if(rejilla.begin(), rejilla.end(), [](const std::pair<const unsigned int, std::unique_ptr<Celda> >& c){return c.second->acc_tipo()==Celda::T_ENTRADA;});
