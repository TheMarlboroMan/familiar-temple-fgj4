...
class Nivel
{
	...
	struct Coordenadas
	{
		...
		unsigned int y;
		Coordenadas():x(0), y(0) {}
		Coordenadas(unsigned int px, unsigned int py):x(px), y(py) {}
	};
	...
	Coordenadas obtener_coordenadas_inicio_en_rejilla() const;
	...
	void convertir_celda_a_deco(unsigned int indice, unsigned int tile);
